postgres15:
	docker run --name postgres15.1 -p 5434:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=mysecret -d postgres:15-alpine
createdb:
	docker exec -it postgres15.1 createdb --username=root --owner=root simple_bank
dropdb:
	docker exec -it postgres15.1 dropdb simple_bank
migrationup:
	migrate -path db/migration -database "postgresql://root:mysecret@localhost:5434/simple_bank?sslmode=disable" -verbose up
migrationup1:
